...
class AcompanhamentoFilaPage extends StatefulWidget {
  final VoidCallback onVoltar;

  const AcompanhamentoFilaPage({
    super.key,
    required this.onVoltar,
  });

  @override
  State<AcompanhamentoFilaPage> createState() => _AcompanhamentoFilaPageState();
}

class _AcompanhamentoFilaPageState extends State<AcompanhamentoFilaPage> {
  final SupabaseClient _supabase = Supabase.instance.client;
  List<Map<String, dynamic>> _fila = [];
  List<Map<String, dynamic>> _filaFiltrada = [];
  ...
  Map<String, dynamic>? _itemFilaSelecionado;
  ...
  Future<void> _aplicarFiltros() async {
    ...
    try {
      ...
      for (var mov in movimentacoesFiltradas) {
        final itemFilaId = mov['ordem_id']?.toString();
        if (itemFilaId == null) continue;

        grupos.putIfAbsent(itemFilaId, () => []);
        grupos[itemFilaId]!.add(mov);
      }

      final List<Map<String, dynamic>> filaResumida = [];
      ...
      for (var entry in grupos.entries) {
        ...
        final produtosAgrupados = agruparProdutosDaFila(entry.value);
      ...
      if (mounted) {
        setState(() {
          _fila = filaResumida;
          _filaFiltrada = List.from(filaResumida);
          _carregando = false;
        });
  ...
  void _aplicarFiltroTexto() {
    ...
    if (termoBusca.isEmpty) {
      setState(() {
        _filaFiltrada = List.from(_fila);
      });
      return;
    }

    final resultado = _fila.where((itemFila) {
      final placasFila = (itemFila['placas'] as List).map((p) => p.toString().toLowerCase()).join(' ');
      if (placasFila.contains(termoBusca)) return true;
      
      final quantidadeTotal = itemFila['quantidade_total'].toString();
      if (quantidadeTotal.contains(termoBusca)) return true;
      
      final statusTexto = _obterStatusTexto(itemFila, null).toLowerCase();
      if (statusTexto.contains(termoBusca)) return true;
      
      final tipoOpTexto = _obterTipoOpTexto(itemFila['tipo_op']?.toString() ?? '').toLowerCase();
      if (tipoOpTexto.contains(termoBusca)) return true;
      
      final produtos = itemFila['produtos_agrupados'] as Map<String, Map<String, double>>;
      if (produtos.keys.any((nome) => nome.toLowerCase().contains(termoBusca))) {
        return true;
      }
      
      return itemFila['itens'].any((item) {
  ...
  Map<String, Map<String, double>> agruparProdutosDaFila(
  ...
  void _abrirDetalhesItemFila(Map<String, dynamic> itemFila) {
    setState(() {
      _itemFilaSelecionado = itemFila;
      _mostrarDetalhes = true;
    });
  }

  Future<void> _cancelarItemFila(Map<String, dynamic> itemFila) async {
    final itemFilaId = itemFila['item_fila_id']?.toString();
    if (itemFilaId == null) return;
    ...
    try {
      await _supabase.from('ordens').delete().eq('id', itemFilaId);

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Item da fila cancelado com sucesso.'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Erro ao cancelar item da fila: $e'),
  ...
  void didChangeDependencies() {
    ...
    // página não está carregando e não possui fila (casos
    // excepcionais ou retorno de navegação).
    if (!_mostrarDetalhes && !_carregando && _fila.isEmpty) {
      _aplicarFiltros();
    }
  }
  ...
  String _obterStatusTexto(Map<String, dynamic> itemFila, Map<String, dynamic>? movimentacao) {
    final item = movimentacao ?? (itemFila['itens'] as List<Map<String, dynamic>>).firstOrNull;
  ...
  Color _obterCorFundoCard(Map<String, dynamic> itemFila) {
    return Colors.white;
  }
  ...
  Widget _buildItemFila(Map<String, dynamic> itemFila, int index) {
    ...
    final produtosAgrupados =
        itemFila['produtos_agrupados'] as Map<String, Map<String, double>>;

    final corFundoCard = _obterCorFundoCard(itemFila);
    ...
    return MouseRegion(
      ...
      child: Container(
        ...
        child: Material(
          ...
          child: InkWell(
            onTap: () {
              _abrirDetalhesItemFila(itemFila);
            },
            borderRadius: BorderRadius.circular(8),
            child: Container(
              ...
              child: Row(
                ...
                children: [
                   // Lado Esquerdo 1: Posição na Fila
                  Container(
                    width: 45,
                  ...
                  // Lado Esquerdo 2: Carga ou Descarga
                  Builder(builder: (context) {
                    final usuario = UsuarioAtual.instance;
                    final terminalAtual = usuario?.nivel == 3 ? _terminalFiltroId : usuario?.terminalId;
                    final itensLocal = itemFila['itens'] as List<dynamic>;
                            ...
                            Expanded(
                              child: Row(
                                children: [
                                  ...
                                  Expanded(
                                    child: Text(
                                      tipoOp == 'transf' && itemFila['motorista_nome'] != null
                                          ? '$placasFormatadas - ${itemFila['motorista_nome']}'
                                          : placasFormatadas,
                                      style: const TextStyle(
                                        fontSize: 12,
  ...
  bool get _isDialog => ModalRoute.of(context) is RawDialogRoute;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        shape: _isDialog ? const RoundedRectangleBorder(
          borderRadius: BorderRadius.only(
        ...
        title: Text(
          _mostrarDetalhes 
              ...
              : 'Acompanhamento de Fila - ${_terminais.firstWhere((f) => f['id'].toString() == _terminalFiltroId, orElse: () => {'nome': ''})['nome'] ?? ''}',
          style: const TextStyle(fontWeight: FontWeight.w600),
        ),
        leading: IconButton(
          icon: Icon(_isDialog && !_mostrarDetalhes ? Icons.close : Icons.arrow_back),
          onPressed: () {
            if (_mostrarDetalhes) {
              _voltarParaLista();
            } else {
                    ...
                    children: [
                      ...
                      Expanded(
                        child: _carregando
                            ...
                            : _erro
                                ? _buildErro()
                                : _filaFiltrada.isEmpty
                                    ? _buildSemDados()
                                    : ListView.builder(
                                        itemCount: _filaFiltrada.length,
                                        itemBuilder: (context, index) {
                                          return _buildItemFila(
                                            _filaFiltrada[index],
                                            index,
                                          );
                                        },
